...
// Attempt initializing Discord RPC
async function initDiscordRPC() {
  try {
    ...
    rpcClient.login({ clientId: CONFIG.CLIENT_ID }).catch((err) => {
...
wss.on('error', (err) => {
  if (err.code === 'EADDRINUSE') {
    console.error(`[Maina RPC] Error: Port ${CONFIG.PORT} is already in use.`);
  } else {
    console.error('[Maina RPC] Server error:', err.message);
  }
});
