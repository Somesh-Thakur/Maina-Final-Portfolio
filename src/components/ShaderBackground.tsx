...
const fragmentShaderSource = `
  ...
  void main() {
    vec2 uv = v_texCoord;
    float speed = u_playing > 0.5 ? 0.08 : 0.03;
    float n = snoise(uv * 1.5 + u_time * speed);
    vec3 finalColor;
    if (u_isDark > 0.5) {
      vec3 baseColor = vec3(0.039, 0.039, 0.039);
      vec3 subtleFlow = vec3(0.06, 0.06, 0.07);
      finalColor = mix(baseColor, subtleFlow, n * 0.5 + 0.5);
    } else {
      vec3 baseColor = vec3(1.0, 1.0, 1.0);
      vec3 subtleFlow = vec3(0.97, 0.97, 0.97);
      finalColor = mix(baseColor, subtleFlow, n * 0.5 + 0.5);
    }
...
export function ShaderBackground({ theme, isPlaying }: ShaderBackgroundProps) {
